...
- 冲突来源必须交叉验证，并说明采信理由。
- 研究业界方案时不复制品牌形态或功能清单；先提取其解决的问题、因果机制、适用前提、主要权衡、失败边界和验证方法，再决定本地采用、调整或拒绝。
...
5. Extract Mechanism：把业界方案压缩为 `问题 -> 机制 -> 前提 -> 权衡 -> 失败边界 -> 验证`，区分可复用指导思想与供应商特定实现。
6. Synthesize：先给结论，再列事实依据和推断链。
7. Apply Locally：说明本项目采纳、调整或不采纳的原因，以及具体进入方案的设计约束或候选模式。
8. Stop：达到停止条件后不继续堆资料。
