...
2026-06-18 周检结论：当前安装来源的 cache-id 已从历史记录中的 `c6ea566d`、同日早前观测到的 `43313cc9` 继续轮换为 `015c0dff`，本地版本仍为 `5.1.3`；官方 `main` 已到 `6.0.2`，但 `skills/` 仍是 14 个 skill，名称集合未变化。因此本映射不新增 Skill owner，只把 v6 的稳定机制刷新到既有 plan/review/parallel coordination/skill authoring references。
...
| `subagent-driven-development` | 任务级 fresh context，规格与质量 verdict，完成后整体复核，按任务复杂度选择能力 | `task-execution-lifecycle`、`code-review` | 对多任务实现采用“规格符合性 -> 代码质量”的行动顺序；v6 的单 reviewer / 双 verdict 模式映射为一次 review 可同时返回 spec 与 quality，但 spec verdict 仍决定是否进入质量动作；`can't verify from diff` 进入证据补强而非通过；任务 brief / review diff 优先用文件或显式上下文包；若计划本身错误，标成 Plan issue；所有子任务完成后做 Final Integration Review；按任务复杂度选择 agent/model/tool 能力，不要求所有平台都有 subagent |
