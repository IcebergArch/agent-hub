...
2026-06-25 周检结论：当前安装来源的 cache-id 已继续轮换为 `7fd3161c`，本地版本仍为 `5.1.3`；官方 `main` 已到 `6.0.3`，`skills/` 仍是 14 个 skill，名称集合未变化。除上周已吸收的 v6 review/plan/delegation 机制外，本周新增的稳定模式是把 subagent scratch/report/progress 文件从受保护的 `.git/` 挪到工作树内的自忽略目录。因此本映射不新增 Skill owner，只补一条 scratch artifact placement 规则并刷新来源证据。
...
| `writing-plans` | 计划要有文件职责、全局约束、任务接口、细粒度步骤、验证命令和自检 | `implementation-plan` | 采用 no-placeholder、自审、Plan Review Gate、Global Constraints、per-task Interfaces 和可执行步骤；不强制写到 `docs/superpowers/` |
...
| `subagent-driven-development` | 任务级 fresh context，规格与质量 verdict，完成后整体复核，按任务复杂度选择能力 | `task-execution-lifecycle`、`code-review` | 对多任务实现采用“规格符合性 -> 代码质量”的行动顺序；v6 的单 reviewer / 双 verdict 模式映射为一次 review 可同时返回 spec 与 quality，但 spec verdict 仍决定是否进入质量动作；`can't verify from diff` 进入证据补强而非通过；task brief / review diff 优先用文件或显式上下文包；若需要落本地 scratch 文件，放在工作树内的自忽略目录，不写进 `.git/`；若计划本身错误，标成 Plan issue；所有子任务完成后做 Final Integration Review；按任务复杂度选择 agent/model/tool 能力，不要求所有平台都有 subagent |
...
| `writing-skills` | Skill 也要用压力场景测试，描述只写触发条件，并让指导形式匹配失败模式 | `methodology-harvest`、`project-content-curation`、`design-principle-library` | 新/改技能先检查触发、真实用户表达/症状/错误/对象/工具/近义词覆盖、抗跑偏、Loaded Surface Token Budget Gate、Cross-Tool Compatibility Gate、token 成本和引用搜索；重规则可设计压力场景验证；必要时用小样本 micro-test 检查措辞是否比 no-guidance 更稳 |
...
| `requesting-code-review/code-reviewer.md`、`receiving-code-review/SKILL.md`、v6 `subagent-driven-development/task-reviewer-prompt.md`、`brainstorming/spec-document-reviewer-prompt.md`、`writing-plans/plan-document-reviewer-prompt.md` | 不原样复制；抽象为 `code-review/references/review-checklists.md`、`implementation-plan/references/plan-review-checklist.md` 和 `task-execution-lifecycle` 的 spec/quality verdict、`can't verify from diff`、Implementation Report Skepticism Gate、Fresh Perspective Review Gate、Feedback Source Priority Gate、Review Verdict Gate、feedback intake、反馈核验、证据型 pushback、错误 pushback 修正、计划 readiness review、Plan issue 出口和 PR thread 处理规则 | prompt 绑定 Superpowers subagent 流程，Hub 先采用审查结构，等真实多 agent 调度稳定后再模板化 |
...
| `subagent-driven-development` 的 `task-brief` / `review-package` / implementer report / progress ledger 文件化交接 | 机制归入 `external-skill-portability.md` 的 scratch artifact placement | Hub 只采用“文件化交接应在工作树内自忽略 scratch 目录、不可依赖 `.git/` 保护路径”的稳定约束；不复制外部 helper 脚本、目录名或恢复流程实现 |
