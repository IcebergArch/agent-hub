...
## Form To Failure Match

修改 Skill/Rule 前先判断失败类型，再选择指导形式；不要把所有问题都写成更多禁止项。

| Failure type | Better form | Avoid |
| --- | --- | --- |
| Agent 明知规则但会找借口绕开 | 明确 red flag、不可接受替代、stop rule、output gate | 只写“应该注意” |
| Agent 不知道什么时候触发 | 更具体的 description、Trigger、反向排除条件、真实用户表达 | 在正文深处补长解释 |
| Agent 输出形态不稳定 | worked example、模板、字段表、verdict 枚举 | 只写抽象原则 |
| Agent 缺工具、权限或上下文 | fallback、unsupported/watch、最小问题、验证替代 | 加重措辞要求它做到 |
| Agent 误解概念边界 | 对比表、owner/destination matrix、positive/negative examples | 堆近义词 |

高频入口优先使用短 stop rule 和输出约束；复杂形态用 reference 或模板承载。

## Micro-Test Wording

重要规则措辞改动可以先做小样本 micro-test：选 3-5 个真实压力场景，对比无新增规则、旧规则和新规则下 Agent 是否更可能触发正确行为。它不是统计证明，但能发现措辞太抽象、过宽触发、误导输出形态或引入新借口。
