...
- 涉及操作型 UI 时，`UX_Product_Design_AGENT` 必须先通过 UX Product Design Quality Gate，再给出布局；若用户要求“研究”“参考业界”“优秀页面实现”或连续指出设计不佳，必须先查看本项目历史文档和现有页面模式，再对照成熟产品的同类管理页/配置页实践，最后给出适配当前任务流的方案；未完成这一步不要直接做视觉微调。
