---
name: codebase-navigation
description: 当用户要求理解项目、业务模型、修改边界、ownership 或影响范围时使用。
---

# Purpose

建立真实项目地图，帮助判断业务模型、owner、修改边界和影响范围。

# When to Use

- “先看仓库/模块”“梳理结构”“功能在哪里”。
- 需要确认业务模型、数据流、入口、依赖方向或不该改的边界。
- 执行前必须理解陌生模块或跨模块影响。

# When NOT to Use

- 目标文件和改动方式已经明确的小修。
- 只需要通用搜索、grep、Git 或 IDE 操作技巧。

# Inputs

- 用户给定路径、目标功能、相关页面/API/命令、已有文档和当前变更边界。

# Decision Principles

- 用户给出的路径或范围是硬边界。
- 文件名相似不等于调用关系；owner 必须由入口、依赖、配置、测试或运行事实支撑。
- 涉及系统性设计或改造时，先按 `系统定位 > 系统架构 > 功能设计 > 领域架构 > 模块设计` 建立事实地图，再决定修改点；局部机械改动只需证明更高层级未受影响。
- 模块边界不由目录或类型名决定，而由调用方必须知道的 interface、行为真正变化的 seam 和改动能否集中到一个 owner 决定；外部 interface 应隐藏实现复杂度，并作为默认测试面。只有存在真实变化、独立 owner/协议边界或生产/测试 adapter 时才新增 seam，不为假想变化制造层级。

# Workflow

1. Boundary：确认用户目标、路径范围和禁止触碰的区域。
2. Entry Model：从最高受影响层开始，确认系统定位与系统边界，再找功能入口、运行入口、页面/API/命令入口和配置入口；未受影响上层用事实标记为 `unchanged`。
3. Ownership：先标出功能与领域 owner、事实源和不变量，再落到模块职责、资源 owner、调用 owner 和测试 owner。
4. Flow：追关键路径上的输入、状态、输出、失败态和外部集成；说明调用方必须知道哪些不变量、顺序、错误和性能特征，以及哪些复杂度应留在 owner 内部。
5. Impact：列出直接影响、间接消费者、兼容风险和需要验证的边界。
6. Decision：给出推荐修改位置、非目标区域和不确定点。

# Checklist

- 是否能说明“为什么这里是 owner”。
- 是否能说明 interface 提供的能力、隐藏的复杂度、真实变化 seam，以及改动与测试是否集中在同一边界。
- 是否区分业务事实源、缓存/registry、展示层和 adapter。
- 是否列出影响范围和验证入口。

# Escalation

- 涉及公共接口或跨服务契约：`skills/Engineering/interface-contract-audit/WORKFLOW.md`
- 项目背景缺失或过期：`skills/Context/project-context-rebuild/WORKFLOW.md`
- 需要外部事实：`skills/Research/source-grounded-research/WORKFLOW.md`

# References

- None.
