...
- `/hub spec 执行 [<SPEC>]`：只消费已有 pending SPEC，或恢复已有执行包的 execing SPEC。首次修改前建立执行包并冻结范围；按真实运行事实完成修改改造和最窄开发验证，结果写入 STDD，最终保持 `execing` 并给出验收入口。IDEA 不是执行候选，实施阶段不得扩写冻结 SPEC，也不得把“代码已改”写成“验收通过”。
