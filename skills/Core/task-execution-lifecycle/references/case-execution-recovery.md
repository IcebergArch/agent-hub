...
- **Configuration First Gate**：用户要求“配置”且已有上传、prompt、preset、skill、tool refs 或页面/API 可以完成目标时，只做配置和可见验证。禁止在 GET 列表接口、普通页面加载、业务初始化入口或后台 service 中偷偷创建业务、prompt、preset、skill 或工具绑定；确有缺口时先给证据和最小改动方案，等用户确认。
