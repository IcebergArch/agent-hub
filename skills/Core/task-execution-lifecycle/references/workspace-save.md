...
`保护现场 -> 登记进度与待办 -> 结束本地服务 -> 整理推送计划 -> 表格确认 -> 执行 -> doc-hub 合入并收拢历史 worktree -> agent-hub canonical main 直接保存并收拢历史现场 -> 表格核验`
...
禁止借 save 执行 reset、checkout、stash、force push、直接 push target、删除或归档任务、丢弃改动、修改数据库或替代项目质量验收；唯一直接 push target 的例外是确认单明确列出的 `agent-hub main -> origin/main` 单分支保存。除本文件定义的 `doc-hub` 历史 linked worktree 和 `agent-hub` 历史隔离现场收拢外，不删除 worktree 或本地分支。
...
- `agent-hub` 固定为 `MAIN`：只在 canonical 主工作目录整理现有 `main` diff，经确认后形成边界清楚的任务提交并直接 push `origin/main`，不创建 work branch 或 PR。历史隔离 worktree/branch 先逐一证明内容已进入 `main` 或已有远端恢复入口、现场为空且无运行任务，再安全删除；任何条件不满足都保留并登记 blocker。
...
| `agent-hub` | `<canonical workspace>` | `main` -> `origin/main` | MAIN + Legacy Cleanup | `main / direct single-branch save` | `<evidence / cleanup blocker>` |
```
...
2. 形成边界清楚的 checkpoint commit，必要时 rebase 最新 target；不 force push、不直接 push target、不用 merge commit 绕过冲突。确认单中的 `agent-hub MAIN` 行按单分支例外直接提交 `main`，fetch 后必须确保可安全更新 `origin/main`，分叉或冲突时停止而不覆盖远端。
3. 按表将每个有未合入内容的隔离空间 push 到其对应远端 work branch。`PR` 行创建或更新对应 PR 后停止；`MR` 行创建或更新 PR，等待 required checks/approvals，并按仓库既有合入规则完成 merge；`MAIN` 行只允许 `agent-hub main -> origin/main`，不得推广到其它仓库。
4. 顺序固定为：其它仓库先处理，之后 `doc-hub` MR 与历史 linked worktree 收拢，最后 `agent-hub` MAIN 与历史现场 Cleanup。不得把这两个 Hub 的清理规则扩散到其它仓库。
...
- 是否按其它仓库 -> doc-hub MR + 历史 linked worktree 收拢 -> agent-hub MAIN + 历史现场 Cleanup 的顺序执行，且未把 Agent Hub 直接 push `main` 的例外扩散到其它仓库。
...
- 是否除已确认的 `agent-hub main -> origin/main` 外没有直接 push target，且没有 force push、stash/reset 或丢弃用户改动。
