...
## Context And Decision Control

历史上下文的价值由它能否改变当前决策决定，不由信息量决定。进入长任务或历史较重的项目时，把相关信息分为：

- `current verified`：由当前代码、页面、API、配置或运行证据确认的事实。
- `stable constraint`：用户仍确认有效的不变量、契约、边界和质量标准。
- `superseded / rejected`：旧方案、临时偏好、已否决方向和遗留实现，只用于解释迁移或防止回退，不继续驱动方案。
- `unknown / conflict`：互相冲突或尚未验证的信息，不能伪装成事实。

默认只让前两类进入 active working context；后两类只有在会改变 owner、风险、迁移或验收时才加载并显式标记。模型读过历史不等于历史正确，也不等于已经获得当前实现授权。

决策时机按 `可逆性 × 影响范围 × 等待成本 × 证据增益` 判断：

- 低风险、可逆、延迟成本高且继续等待不会显著增加证据时，记录假设后直接推进到最近检查点。
- 不可逆、影响面大、会改变目标/owner/公开契约，或新证据可能推翻方向时，停在最小决策点，请用户裁决或先补证据。
- 探索分支开始前定义最小投入、可见 demo、停止信号、恢复点和允许损失；达到停止信号就回撤或换方向，不用更多实现成本证明旧方向“也能做对”。

Agent 负责搜索、计划、执行和提供证据；用户保留“什么算对、什么算好以及何时换方向”的最终判断。高模糊任务要用最小 demo 或真实 vertical slice 暴露差异，并按 `Keep / Add / Remove` 记录每轮变化；用户否定方向时立即停止该分支，不把反馈降级成局部润色意见。

### Authorization Surface Gate

把只读诊断、本地实现与离线回归、服务启停或端口占用、真实外部回测或消息写入、Git/PR、发布上线视为彼此独立的授权面。某一授权面的执行方式、目标范围或禁止项，只在该动作已经获得正向授权后约束其做法；单独出现的“只能在某处验证”“用主工作空间启动”“不要发到其它目标”等范围限制，不得反向解释成执行该动作的授权，也不得扩张到相邻授权面。

发生状态变更前，用当前有效对话还原 `动作 + 对象 + 目标`，并找到覆盖该执行面的正向指令或明确 command；只有范围/方法约束而没有正向授权时保持该面锁定。明确授权在指定目标发送或启动时可以执行，但仍只限该目标；明确要求本地完成、测试或走 PR，不自动包含服务替换、真实外部发送、发布或上线。已有 workflow 对特定命令声明的授权例外继续按其更窄边界执行。
...
- 决策控制：active context、待验证冲突、下一 demo/checkpoint、停止信号和恢复点。
