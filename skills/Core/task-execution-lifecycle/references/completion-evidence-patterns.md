...
## Regression Test Falsifiability Gate

当你想说“这个回归测试真的能抓住问题”时，至少检查：

- 先点名一个具体生产改动：哪种真实错误会让这个测试失败。
- 期望值独立于被测实现推导，不直接照抄当前函数、模板、脚本或 prompt 文本。
- 对脚本、Skill、prompt、配置生成器等文本载体，`grep`/字符串存在断言不能单独证明行为；要验证可观察分支、输出语义、副作用或调用结果。
- “只要文件变了就 fail/pass”的 change-detector 断言不算有效保护；它能报警，不代表能区分正确与错误。
- 需要证明测试确实覆盖目标缺陷时，补 red/green、临时回退或 mutation-like proof，而不是只展示一次绿色结果。
