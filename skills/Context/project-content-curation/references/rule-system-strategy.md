...
规则体系的目标不是把所有好规则放进一个入口，而是让 Agent 用最少上下文读到正确层级：入口文件只保留硬约束和触发索引；项目关键事实进入文档工作区 helper；可重复组织经验进入 Skill；长步骤、矩阵、示例进入 reference；需要长期引用的历史证据进入正式报告；临时迁移账本、缓存路径和一次性对照进入 Documents temp。

## Agent-Facing Document Quality Gate

面向 Agent 的入口、Skill 和 reference 不只要内容正确，还要让触发与执行稳定：

- **触发指针**：入口中的一句引用、Skill description 或升级路径必须同时说明“这是什么”和“哪些分支需要读取”；触发条件优先写在前面，同一分支的近义触发词要合并。目标文件再完整，弱指针也会造成漏读。
- **信息层级**：当前路径必经的动作留在正文；所有分支都需要的短规则可留在同文件；只有特定分支需要的长 reference 放到指针后。定义、规则和例外应就近放在同一主题下，避免同一概念散落多处。
- **完成条件**：容易提前结束或质量波动大的步骤，要有可观察且足够完整的完成条件，例如“每个修改 owner 均有验证证据”，而不是“理解完成”。优先收紧完成边界；只有后续步骤本身持续诱发抢跑时，才按真实阶段或上下文边界拆分。
- **两种负载**：默认上下文中的每句话消耗 Agent 的上下文与注意力；新增文件、命令或入口增加人的查找负担。拆分只有在降低触发误差或执行波动的收益大于新增 owner 成本时才成立。
- **环境是真实来源**：目录、配置、脚本、命令帮助和生成链路能低成本直接查到时，文档重复它们只是易过期缓存。文档只缓存昂贵查询、未写明惯例、设计理由和无法从环境直接发现的陷阱。
- **正向目标与 no-op**：优先写 Agent 应达到的目标行为；只有无法正向表达的硬护栏才保留禁止语，并同时给出安全替代。逐句检查是否真的改变默认行为；不能改变行为、无法触发或无法验证的句子直接删除。

## Problem-Solving Priority

解决问题和沉淀规则时，默认按以下顺序取舍：

1. **抓主要问题**：先定位当前任务真正阻塞目标、破坏边界或影响用户验收的主问题；支线信息只在会改变决策时加载或沉淀。
2. **实事求是**：判断必须基于真实需求、上下文、证据、能力边界和产物状态；不能用抽象规则、历史偏好或理想流程覆盖当前事实。
3. **高效**：优先选择最小足够闭环，用最少读取、最小 diff 和最窄有效验证解决问题；不为形式完整扩写规则或流程。
4. **长远视角**：短期处理不能破坏 owner、架构分层、可验证性和后续维护；只有跨场景稳定、可触发、可验证的不变量才进入长期规则体系。
...
| `skills/<Category>/<skill-slug>/WORKFLOW.md` | 可重复触发的组织经验：Purpose、适用/不适用场景、输入、决策原则、流程、检查项和升级路径 | Codex 默认能力教程、大量示例、长命令、历史背景、项目私有事实 |
| `skills/<Category>/<skill-slug>/references/` | 该 Skill 的长检查表、矩阵、模式、执行细则、可选深读 | 与 Skill 无关的项目结论、通用入口规则 |
...
7. **索引入口**：保留在 README、skills index、project helper 或 report artifact index。
...
验证触发指针时至少回放原场景、一个不应触发的相邻场景和一个需要进入不同 reference 的分支；验证 workflow 时检查高波动步骤是否有可观察完成条件。
